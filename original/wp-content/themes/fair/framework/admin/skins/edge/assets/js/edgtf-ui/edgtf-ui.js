			...
			frame: function() {
				...
				this._frame.on('update', function() {
					...
					jQuery.ajax({
						type: "post",
						url: ajaxurl,
						data: "action=fair_edge_gallery_upload_get_images&ids=" + ids,
...
//            }
	...
	function edgtfImportBackupedOptions(){
		...
		if($('.edgtf-backup-options-page-holder').length) {
			...
			edgtfImportBtn.on('click', function(e) {
				...
				if (confirm('Are you sure, you want to import Options now?')) {
					...
					var data = {
						action: 'fair_edge_import_theme_options',
						content: importValue,
						nonce: importNonce
					};
					$.ajax({
